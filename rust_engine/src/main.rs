...
async fn record_rtds_btc_tapes(
    ...
    const SOURCE_STALE_AFTER: Duration = Duration::from_secs(20);
    ...
    let mut chainlink_writer = std::io::BufWriter::new(
        std::fs::File::create(&chainlink_tmp)
            .with_context(|| format!("create {}", chainlink_tmp.display()))?,
    );
    let mut binance_writer = std::io::BufWriter::new(
        std::fs::File::create(&binance_tmp)
            .with_context(|| format!("create {}", binance_tmp.display()))?,
    );
    writeln!(chainlink_writer, "timestamp_ms,source,price,received_at_ms")?;
    writeln!(binance_writer, "timestamp_ms,source,price,received_at_ms")?;
    ...
    while started.elapsed() < capture_for {
        ...
        let mut last_chainlink_tick = Instant::now();
        let mut last_binance_tick = Instant::now();
        ...
        loop {
            ...
            tokio::select! {
                ...
                _ = ping.tick() => {
                    let chainlink_stale = last_chainlink_tick.elapsed() > SOURCE_STALE_AFTER;
                    let binance_stale = last_binance_tick.elapsed() > SOURCE_STALE_AFTER;
                    if chainlink_stale || binance_stale {
                        stats.idle_timeouts += 1;
                        stats.chainlink_idle_timeouts += u64::from(chainlink_stale);
                        stats.binance_idle_timeouts += u64::from(binance_stale);
                        break;
                    }
                ...
                message = read.next() => match message {
                    Some(Ok(Message::Text(text))) => {
                        ...
                        for row in rows {
                            let writer = if row.source == "chainlink_btc_usd_data_stream" {
                                last_chainlink_tick = Instant::now();
                                stats.chainlink.observe(row.timestamp_ms, received_ms);
                                &mut chainlink_writer
                            } else {
                                last_binance_tick = Instant::now();
                                stats.binance.observe(row.timestamp_ms, received_ms);
                                &mut binance_writer
                            };
                            writeln!(
                                writer,
    ...
    chainlink_writer.flush()?;
    binance_writer.flush()?;
    drop(chainlink_writer);
    drop(binance_writer);
    std::fs::rename(&chainlink_tmp, &chainlink_path).with_context(|| {
        format!(
            "rename {} to {}",
            chainlink_tmp.display(),
            chainlink_path.display()
        )
    })?;
    std::fs::rename(&binance_tmp, &binance_path).with_context(|| {
        format!(
            "rename {} to {}",
            binance_tmp.display(),
            binance_path.display()
        )
    })?;
